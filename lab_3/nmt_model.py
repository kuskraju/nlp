...
class NMT(nn.Module):
    ...
    def encode(
        ...
        init_decoder_hidden = self.h_projection(torch.cat(torch.split(last_hidden, 1, dim=0), dim=2).squeeze(dim=0))
        init_decoder_cell = self.c_projection(torch.cat(torch.split(last_cell, 1, dim=0), dim=2).squeeze(dim=0))
        dec_init_state = (init_decoder_hidden.squeeze(dim=0), init_decoder_cell.squeeze(dim=0))
        # TODO:
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t = torch.bmm(enc_hiddens_proj, torch.unsqueeze(dec_hidden, 2)).squeeze(dim=2)
